...
while tmpnumber > 0:
    ...
    print('the sum of the digits of',number,'is',SumOfDigits)
...
while i < len(listOfWords):
    if len(listOfWords[i]) > wordLenght:
        longWords+=1
    elif len(listOfWords[i]) < wordLenght:
        shortWords+=1

    i+=1

print('Words shorter then ',wordLenght,':',shortWords)
print('Words longer then ',wordLenght,':',longWords)
